...
matches Hebrew lexical forms at rates 1.7--3.4 times above random baselines
...
and outperforms a synthetic null model on match rate, Zipfian gloss
distribution, and Hebrew bigram plausibility.

A known-plaintext crib attack reverses the traditional pipeline: we generate
plausible Hebrew medical texts via an LLM, encode them through the cipher,
and compare against real manuscript text. This produces 1{,}758 exact matches
across all 227~folios ($\sim$5\% of tokens; 78.7\% average match rate on
132~herbal pages including pre-existing anchors), while three independent
controls --- random Hebrew words (0/176{,}600), cross-folio transfer
(0/410{,}826), and shuffled positions (0/151{,}000) --- all return zero
matches ($z>100$). Near-match analysis reveals systematic character
substitution patterns (vav/resh, tet/tav, ayin/chet) consistent with medieval
Hebrew paleography. An LLM-assisted interpretation of the decoded consonants
suggests pharmaceutical content, though this interpretation is conditioned
on section type and has not been independently verified.

Despite these results, the text remains unreadable: DictaBERT syntax
scores (2.74/6) are barely above random consonant strings, botanical
vocabulary shows anti-concentration in herbal sections ($z=-6.0$),
and zodiac validation achieves only 1 of 12 expected sign matches.
A length-stratified analysis reveals that the signal is concentrated
in 3--4 letter words ($z=8.14$ at length~4, $7.8\times$ ratio) while
2-letter matches are indistinguishable from chance ($z=0.01$) and
5+ letter matches collapse to 0.4\%. A Hebrew prefix frequency test
finds 0/7 grammatical prefixes (bet, kaf, lamed, mem, shin, he, vav)
in their expected frequency ranges --- shin appears at 41.3\% of
word-initial positions versus an expected 1--5\%.
The mapping captures a real lexical signal at short word lengths
whose grammatical and linguistic interpretation remains open.
\end{abstract}
...
We organize 35 systematic investigations across mapping derivation
...
independently through Italian phonemic transliteration.\footnote{The
  Hebrew and Italian optimization paths converged on 84.2\% (16/19)
  of characters.  A convergence control using a simplified steepest-ascent
  hill-climber (10 restarts) on shuffled text and random strings yields
  0--12\% convergence (0--2/17 characters agreeing with the published
  mapping), consistent with the chance expectation ($1/22 \approx 4.5\%$
  per character). The 84.2\% agreement between independent derivation
  paths greatly exceeds any control.}
...
recovered two additional Hebrew letters:
  ...
  \end{itemize}
These splits were selected from 36 explicitly tested hypotheses (6
anomalous position--letter pairs $\times$ 5 unmapped Hebrew letters,
plus 6 allographic variants). The full search space comprises
$19 \times 3 \times 5 = 285$ possible splits. Both adopted splits
survive Bonferroni correction at $\alpha/285 = 1.75 \times 10^{-4}$:
\eva{n}$\to$bet $z = 40.1$, $p_{\mathrm{corrected}} < 10^{-100}$;
\eva{r}$\to$samekh $z = 19.5$, $p_{\mathrm{corrected}} < 10^{-80}$
(sign test on gained vs.\ lost matches).\footnote{The
  sign-test $z$-scores treat each gained or lost match as an
  independent Bernoulli trial under $H_0$: $P(\text{gain}) =
  P(\text{loss}) = 0.5$.  Bonferroni correction for the full
  285-hypothesis search space leaves significance unchanged.}
\end{enumerate}
...
To address the concern that the mapping was derived and validated on the
same data, we performed two cross-validation experiments using the honest
lexicon (45K forms).

\textbf{Hand-based split.}
Hand~1 pages (6,832 tokens) serve as the training set; all other hands
(30,193 tokens) form the test set. We ran the constrained per-letter
audit independently on each subset. Full held-out derivation
(re-running the hill-climbing optimizer on each subset) would require
re-implementing the historical 10-group homophonic architecture; instead,
we test whether the existing mapping is \emph{independently optimal} on
each data partition. Results: 15/17 letters optimal on training, 13/17
on test, with 13/17 independently optimal on both subsets.
The four non-shared letters (\eva{c}, \eva{g}, \eva{m}, \eva{t}) show
small gaps ($\le$137 tokens).
The permutation test ($n=200$) on the held-out
(non-Hand~1) subset yields \zs{5.72} (\pval{0.005}).

\textbf{Random 50/50 split.}
We randomly partitioned pages into two halves (10 repetitions, different
random seeds). For each split we audited mapping optimality on the training
half and computed a permutation $z$-score on the test half.
All 10 splits are significant: mean \zs{5.85 \pm 0.32} (range 5.36--6.36),
mean optimality 13.5/17 (range 13--14), test match rate $25.8\% \pm 0.7\%$.

A mapping that achieves \zs{>5} on held-out data across all splits, with
13/17 letters independently optimal on disjoint subsets, cannot be
dismissed as an artefact of overfitting to one corpus partition.
...
\item Even the smallest lexicon (STEPBible, 5,475 biblical forms) yields
...
\midrule
Match rate & 21.48\%\footnote{The honest match rate differs between the null model test (21.48\%, 33,320 tokens after layout filtering) and the mapping audit (25.6\%, 36,922 tokens including all layout types). Both values are correct for their respective corpora.} & 8.91\%$\pm$0.13\% & 98.2 & significant \\
...
However, it should be noted that the null model is deliberately minimal:
it preserves only marginal character frequencies and word lengths, discarding
all sequential structure (bigram, trigram, and positional patterns). Any
systematically derived mapping would preserve the EVA text's sequential
structure when projected onto the Hebrew alphabet, and would therefore
outperform this baseline. The null model thus provides a \emph{floor}
(the decoded text is better than random character sequences), not a
ceiling. The permutation tests in \S\ref{sec:permutation}, which compare
against random bijective mappings preserving all source-text structure,
...
\item Only 30 types match Arabic but not Hebrew (491K); examples include
  \emph{sryr} (\textit{sar\={\i}r}, ``bed,'' freq=103) and \emph{Srwr} (\textit{shur\=ur}, ``malice'')
\end{itemize}
...
higher match rates (full lexicon): 45.7\% vs.\ 38.7\% ($+7.0$~pp, $z=11.64$, $p<0.0001$).
...
(Language~B, 24.9\%). In paragraph text overall,
Hand~1 (28.8\%) vs.\ Hand~2 (24.9\%): $+3.9$~pp, $z=3.12$, $p<0.002$.
A content-controlled comparison restricted to herbal pages (full lexicon)
shows an even larger gap: Hand~1 48.6\% vs.\ Hand~2 38.6\%
($+9.9$~pp, $z=8.18$, $p<0.0001$).

A per-character frequency comparison (proportion $z$-test with Bonferroni
correction for 19 characters, $\alpha_{\mathrm{adj}} = 0.0026$)
reveals the structural source of the gap. Hand~4 uses EVA~\eva{e}
at roughly twice the rate of Hand~1 (11.1\% vs.\ 5.3\%), making it the
single largest divergence. Because \eva{e}$\to$pe occurs
disproportionately in non-matching contexts, this alone accounts for a
substantial portion of the match-rate difference. Hand~4's small sample
size (765 paragraph tokens) limits statistical power, with only 2--3
characters reaching Bonferroni significance. The full 19-character
comparison is available in the supplementary data.
...
A\footnote{Section code ``A'' denotes a subset of astronomical pages in the IVTFF transcription, distinct from the main astronomical section (S).} & --- & 12.9 & 18.5 & +5.6 & 259 \\
Z & zodiac & 12.2 & --- & --- & 0 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Domain-Specific Lexicon Test}
\label{sec:domain-lexicon}

To test whether the decoded text exhibits domain specificity --- a requirement
for meaningful decipherment --- we assembled a curated lexicon of 420 medieval
Hebrew terms across four domains: botanical (148 terms from Shem Tov and Talmud),
astronomical (76 from ibn Ezra), medical (96 from Asaph ha-Rofe and medieval
translations), and balneological (64 from Mishnah Mikvaot and medieval sources).
For each domain, we counted decoded matches per manuscript section and tested
whether matches concentrate in the expected sections (e.g., botanical terms
in herbal/pharmaceutical) via both chi-square and permutation tests
(1,000 permutations of section labels).

\begin{table}[htbp]
\centering
\caption{Domain lexicon concentration by manuscript section. Concentration
ratio $>1$ indicates above-baseline concentration in expected sections.}
\label{tab:domain}
\small
...
\midrule
Balneological & B & 649 & \textbf{2.08} & 8.53 & $<$0.001 \\
...
decoded water/bathing terms concentrate at $2.08\times$ baseline in the
balneological section ($z=8.53$, $p<0.001$). Astronomical and medical
domains are also significant by permutation but with weak or dispersed
concentration. Botanical terms, surprisingly, show \emph{anti-concentration}
($z=-6.00$): herbal sections contain \emph{fewer} botanical matches per
word than other sections. This negative result is consistent with the
section-entropy finding (\S\ref{sec:layout}) that the same high-frequency
glosses dominate all sections, with no domain specialization in decoded
vocabulary.

\subsection{Scribal Error Correction}
\label{sec:scribal-errors}

Approximately 75\% of decoded word types do not match the honest lexicon.
We tested whether single-character scribal errors --- visually confusable
EVA glyph substitutions --- could account for part of this gap.  Eleven
confusion pairs were defined from EVA glyph shape analysis (a/o, e/i, n/r,
c/e, d/s, k/t, f/p, l/r, m/n, i/n, c/h).  For each of the 7{,}933
unmatched types, we generated all distance-1 visual variants (substituting
one confusable glyph at a time), decoded each variant, and checked against
the honest lexicon (45K forms).

This procedure recovered 835 types (10.5\% of unmatched) corresponding to
8{,}544 tokens (31.1\%).  The most productive substitution was o$\to$a
(2{,}077 tokens), followed by a$\to$o (1{,}108) and n$\to$i (793).
Recovery rates were uniform across scribal hands (9.6\%--15.6\%) and
manuscript sections (9.3\%--13.9\%), with no hand or section standing out.

A permutation control measured whether the \emph{specific} confusion pairs
outperform arbitrary ones.  We shuffled the confusion-pair labels 200 times,
each time applying the same correction procedure.  The result is a clear
null: type $z=-0.07$ ($p=0.53$), token $z=-0.03$ ($p=0.51$).  Shuffled
pairs recover the same number of words as the real visual-confusion pairs.
The recovery is therefore an artefact of lexicon density --- any
single-character substitution has a non-trivial chance of hitting the
45K-form lexicon --- rather than evidence of actual scribal copying errors.

This null result has a constructive implication: the 75\% unmatched words
cannot be recovered by error correction against the current lexicon.  The
gap reflects \textbf{lexicon inadequacy} --- medieval Hebrew technical
vocabulary (botanical, astronomical, medical) is underrepresented in the
biblical and talmudic sources that constitute the honest lexicon --- rather
than scribal noise.  Progress toward readability requires a better-suited
lexicon, not error correction.
...
\item Structurally embedded in character sequences beyond marginal frequency overlap
...
An epistemological tension deserves acknowledgment: DiPrima's framework
assumes the Voynich text represents non-linguistic control programs
(distillation instructions), yet his distributional classifications
successfully predict our Hebrew lexical match rates. This convergence
admits two interpretations: (a)~the linguistic and distributional structures
are genuinely correlated --- both methods detect the same underlying
organization of the text; or (b)~both systems detect shared statistical
regularities (word-length distributions, positional character patterns)
without either correctly identifying the generative mechanism. The
cross-analysis demonstrates that \emph{something real} is being captured
by independent methods, but does not adjudicate between linguistic and
non-linguistic interpretations of that signal.
...
Token counts reflect the 16,100 matched tokens (of 37,025 corpus tokens);
token-weighted rates show the higher frequency of confirmed forms.}
\label{tab:dictalm}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
\subsection{Syntactic Analysis with DictaBERT}
\label{sec:dictabert}

To move beyond generative LLM assessment, we applied DictaBERT-parse
\citep{DictaBERTParse2024}, a transformer-based Hebrew dependency parser
trained on Universal Dependencies treebanks. Unlike DictaLM, this model
produces structured syntactic analysis: POS tags, morphological features,
and dependency trees. We computed a syntactic quality score (0--6) based
on the presence of a root, verb, noun, function words, core arguments
(subject/object), and dependency diversity.

We selected the 47 best consecutive 5-word windows in which all words are
DictaLM-valid --- by construction, the most favorable windows in the entire
corpus. This deliberately optimistic selection means that the results below
represent an \emph{upper bound} on syntactic quality
(Table~\ref{tab:dictabert}).

\begin{table}[htbp]
\centering
\caption{DictaBERT syntactic quality scores (0--6 scale) for three text
types. Permutation $z$ compares real word order against random reorderings
...
\midrule
\multicolumn{2}{@{}l}{Permutation test ($z$)} & $-0.51$ (real order $\leq$ random) \\
\bottomrule
\end{tabular}
\end{table}

Even under this optimistic selection, only $23\%$ of Voynich phrases
receive a VERB tag (vs.\ $100\%$ for real Hebrew), $13\%$ have function
words, and only $2\%$ (1 phrase) have core syntactic arguments.

A permutation test comparing the real consecutive phrases against random
sequences drawn from the same word vocabulary yields $z = -0.51$: the
actual word order scores \emph{lower} than random orderings. This rules
out the possibility that the decoded text contains latent syntactic
structure not captured by the lexical analysis.

The parser treats most decoded tokens as NOUN chains in construct state
(\emph{smixut}), which is the Hebrew parser's default for sequences of
unrelated nominals. The conclusion is unambiguous: \textbf{the signal is
purely lexical; word ordering carries no detectable syntactic information.}
...
medical terms --- is poorly represented. A scribal error correction
experiment (\S\ref{sec:scribal-errors}) confirms that the 75\% unmatched
gap is not recoverable through single-character corrections; the shortfall
is lexical, not scribal. However, a supplementary test using 96{,}447 new
consonantal forms extracted from medieval Rishonim texts (Sefaria
\texttt{hebrew\_library} corpus: Halakhah, Responsa, Jewish Thought,
Kabbalah) yielded only $+58$ additional decoded matches ($+760$ tokens),
suggesting the gap is not merely a matter of lexicon period mismatch.
...
mappings is vast ($22!/3! \approx 10^{20}$); our mapping is locally optimal
...
\item \textbf{DictaLM calibration}: the 1.7B-parameter model used for
Hebrew validation has limited capacity. A blinded calibration experiment
(100 known Hebrew forms, 100 random consonantal strings, 100 Voynich
forms, submitted without origin labels) yielded 100\% precision but only
2\% recall (strict): the model recognized only 2/100 known Hebrew words
and 0/100 random strings (FPR\,=\,0\%). Of 100 blinded Voynich forms,
3 were accepted (vs.\ 0 expected by chance).
The extremely low recall means the model is too conservative to serve as
a reliable standalone validator; the ``possible'' category (70.1\% of
Voynich forms in the main experiment) reflects this conservatism rather
than genuine ambiguity. The $z=56.71$ DictaLM-filtered score should be
interpreted as a lower bound, since the model rejects most valid Hebrew.
\end{itemize}
...
\midrule
Alphabet size & 22\footnote{The EVA transcription uses 22 distinct characters including rare and uncertain forms; our mapping uses the 19 principal characters that account for $>$99\% of the text.} & 19 & 22 \\
...
closing approximately 20\% of the gap between raw EVA and real Hebrew
($3.72 - 2.12 = 1.60$ bits). The direction is consistent with partial
decipherment: a correct cipher key should decompress the entropy of the
ciphertext toward natural-language values. However, the magnitude is
modest --- the decoded text remains much closer to raw EVA than to the
Hebrew reference ($h_2 = 2.44$ vs.\ $3.72$), and the 80\% residual gap
confirms that the mapping is, at best, a partial decipherment.
...
\citet{Cheshire2019} & Proto-Romance & Italian 5\% vs Hebrew 25\% &
  \textbf{R} \\
\citet{Greshko2025} & Naibbe cipher & $z=12.1$, 8/9 mono & \textbf{T} \\
\citet{Rugg2004} & Cardan grille (hoax) & Perm.\ $z=3.6$--$4.4$ &
  \textbf{T} \\
...
\citet{Currier1976} & Two languages A/B & Both sig.; A$=$Hand~1 &
  \textbf{C+} \\
\citet{Timm2014} & Non-linguistic mechanism & Perm.\ $z=3.6$--$4.4$ &
  \textbf{T} \\
...
extends, R = refutes, T = in tension, X = contradicts, N = neutral.}
\end{tabular}
\end{table}

Of the 15 comparisons, \textbf{8 confirm} our results (including two that
we extend with new scribe-level and Currier-level data), \textbf{1 is
refuted} by our evidence (Cheshire's Proto-Romance hypothesis, contradicted
by the 5\% Italian vs.\ 25\% Hebrew match rate), \textbf{4 are in tension}
with our findings (our data provide evidence against these claims but do not
conclusively exclude them), \textbf{1 is contradicted}, and \textbf{1 is
neutral}.
...
result shows that the mapping partially shifts $h_2$ toward
natural-language values: an increase of 0.32 bits, closing approximately
20\% of the gap to the Hebrew reference (3.72 bits). This is consistent
with the mapping capturing some cipher structure, but the modest magnitude
(80\% of the gap remains) means the evidence is suggestive rather than
conclusive.
...
\section{Crib Attack: Known-Plaintext Validation}
\label{sec:crib-attack}

The preceding analysis established that the mapping produces real Hebrew
vocabulary but no readable text. In this section we reverse the approach:
instead of decoding and hoping for sense, we \emph{generate} plausible
Hebrew texts, encode them through the cipher, and compare against the
real manuscript.

\subsection{Method}

For each folio, the pipeline (1)~extracts word-length constraints and
anchor words (known glosses) from the EVA transcription,
(2)~prompts Claude Sonnet to generate 10~Hebrew consonantal candidates
per unknown word position, conditioned on section type (herbal,
balneological, zodiac, astronomical, cosmological, pharmaceutical, text)
and surrounding anchors, (3)~encodes each candidate through the cipher
and computes edit distance to the real EVA, and (4)~collects exact matches
($d=0$) and near-matches ($d\leq 1$).

A \textbf{cross-page bootstrap} mechanism feeds exact matches confirmed
on 2+ independent pages back as strong anchors for subsequent rounds
(leave-one-out to prevent circularity).

\subsection{Results}

We processed all 227 folios of the manuscript. After two bootstrap rounds
on the 132~herbal pages:
...
Metric & Value \\
\midrule
Herbal pages (132 folios) & 78.7\% avg coverage \\
Pages $\geq 90\%$ & 15 \\
Pages $\geq 80\%$ & 65 \\
Pages $< 50\%$ & 0 \\
Exact matches ($d=0$) & 1{,}758 \\
Unique Hebrew words confirmed & 1{,}149 \\
Cross-page vocabulary ($\geq$2 pages) & 280 words \\
\bottomrule
\end{tabular}
\end{center}

Non-herbal sections show variable performance: text-only pages (T) reach
80.0\%, zodiac (Z)~63.6\%, balneological (B)~57.2\%, cosmological (C)~55.1\%,
astronomical (S)~42.9\%, pharmaceutical (P)~37.1\%.

\subsection{Validation Controls}

Three independent controls were performed to rule out artefactual matching:

\begin{enumerate}
\item \textbf{Random Hebrew words}: Each matched position was replaced with
a real Hebrew word of the same length drawn from the full 357{,}465-form
lexicon. Result: \textbf{0 matches in 176{,}600 attempts} (200~permutations).

\item \textbf{Cross-folio}: Hebrew candidates generated for page~A were
tested against EVA text on page~B. Result: \textbf{0 matches in 410{,}826
attempts}. This rules out LLM bias --- if candidates were generic ``good
Hebrew'', they would match other pages.

\item \textbf{Shuffled positions}: Within each page, EVA words were permuted
while keeping the matched Hebrew words fixed. Result: \textbf{0 matches in
151{,}000 attempts}. The signal is position-specific.
\end{enumerate}

All three controls return exactly zero, yielding $z>100$ and ruling out
chance alignment at any conventional significance level.

\subsection{Circularity Concerns}
\label{sec:crib-circularity}

Several aspects of the crib attack design introduce potential circularity:
(1)~the LLM is conditioned on section type (herbal, balneological, etc.),
which biases generated content toward domain-appropriate vocabulary;
(2)~coverage percentages include pre-existing anchor words (e.g., on
folio~27r: 68\% anchors, 17\% new exact matches, 16\% near-matches);
(3)~the bootstrap mechanism feeds confirmed matches back as anchors,
potentially inflating subsequent rounds.
The zero-controls rule out random coincidence but do not address whether
the LLM is ``finding'' manuscript content or ``generating'' plausible
content that happens to match. Independent expert verification ---
ideally by a Hebraist reading the decoded consonants without LLM
assistance --- is required to establish semantic validity.

\subsection{Scribal Confusion Patterns}

Analysis of 599 near-matches ($d=1$) reveals systematic character
substitution patterns consistent with known medieval Hebrew paleography:
\eva{w}$\leftrightarrow$\eva{r} ($\times$26, vav/resh confusion),
\eva{J}$\leftrightarrow$\eva{t} ($\times$25, tet/tav phonetic merger),
\eva{A}$\leftrightarrow$\eva{h} ($\times$24, aleph/he confusion),
\eva{E}$\leftrightarrow$\eva{X} ($\times$19, ayin/chet guttural confusion).
These are all documented in medieval Hebrew manuscript studies and are
consistent with a Hebrew scribal origin, though they could also arise
if the mapping is partially incorrect in ways that happen to align with
known paleographic confusions.

\subsection{Section-Specific Vocabulary}

Astronomical pages (S) show a distinctive morphological pattern: 6.3\% of
words carry an EVA \eva{l}-prefix (Hebrew mem-suffix, indicating plural
forms), compared to only 0.7\% in herbal pages --- a $9\times$ enrichment.
This suggests the astronomical sections employ a different grammatical
register (plural/collective forms vs.\ singular in recipes), which may
account for their lower crib-attack performance when using herbal-derived
vocabulary.

\subsection{Content}

An LLM-assisted interpretation of the decoded herbal pages suggests
pharmaceutical extraction recipes: filtering through cotton (\heb{mwk}),
pouring (\heb{Spk}), clarifying with chalk (\heb{gyr}), extraction
with alcohol (\heb{Skr}), and a completion test of bitter (\heb{mr})
becoming sweet (\heb{mtq}). Balneological pages are interpreted as
therapeutic bathing protocols. This content is consistent with medieval
Judeo-Italian pharmaceutical manuscripts, but the interpretation is
conditioned on section type (see \S\ref{sec:crib-circularity} above)
and has not been verified by an independent Hebraist.
...
testing and is in tension with tested alternative explanations
(homophonic cipher, Aramaic, Italian, Judeo-Italian, Judeo-Arabic, Ladino),
though it does not conclusively exclude all non-Hebrew generative mechanisms.

A known-plaintext crib attack (\S\ref{sec:crib-attack}) confirms the
non-random nature of the cipher relationship: generating plausible Hebrew
candidates and encoding them through the cipher produces 1{,}758~exact
matches across 227~folios ($\sim$5\% of tokens; 78.7\% average match
rate on herbal pages including pre-existing anchors), while three
independent controls (random words, cross-folio, shuffled positions) all
return zero matches. However, the crib attack's semantic interpretations
are conditioned on section type and have not been independently verified
(\S\ref{sec:crib-circularity}). A domain-specific lexicon test
(\S\ref{sec:domain-lexicon}) finds one significant result --- balneological
vocabulary concentrates at $2.08\times$ baseline in the bathing section
($z=8.53$) --- but botanical terms show \emph{anti}-concentration
($z=-6.0$) and zodiac validation achieves only 1 of 12 expected sign
matches ($p=0.071$, not significant). DictaBERT syntax analysis confirms
that word order is indistinguishable from random ($z=-0.51$, score
2.74/6). The signal is concentrated in one scribal hand and in continuous
text rather than figure labels.

A length-stratified permutation test reveals that the signal is driven
by 3--4 letter words: $z=3.62$ at length~3 ($2.3\times$ ratio) and
$z=8.14$ at length~4 ($7.8\times$ ratio), while 2-letter matches are
indistinguishable from chance ($z=0.01$, 56.6\% real vs.\ 56.4\%
random) and 5+ letter matches collapse to 0.4\% (71 of 16{,}804
tokens, $p=0.055$). A Hebrew grammatical prefix analysis finds 0 of 7
expected prefixes (bet, kaf, lamed, mem, shin, he, vav) in their
canonical frequency ranges: shin appears at 41.3\% of word-initial
positions (expected 1--5\%), the definite article he is absent
(expected 5--12\%), and lamed appears at 0.1\% (expected 4--10\%).
The decoded text therefore has \textbf{no Hebrew grammatical structure}
despite its lexical signal at short word lengths.
...
consonantal morphology at triconsonantal root lengths. Cross-analysis with an independent distributional
...
across the text's internal structure. Distinguishing between these possibilities requires the
...
mapping captures some real Hebrew vocabulary: 19.4\% of matched types are
confirmed as genuine Hebrew words (211 of 1{,}088 forms). A permutation
test restricted to these confirmed forms yields $z=56.71$, but this
result is selected on the highest-confidence subset; including all
decoded forms reduces the $z$-score to 3.70. DictaBERT syntax scoring
(2.74/6, where random consonants score 2.5/6 and real Hebrew scores
$\sim$6.0/6) confirms that the decoded text has lexical matches but
no syntactic structure.
The overwhelming medieval/rabbinic period classification of confirmed words
suggested that a medieval Hebrew lexicon might close the gap; however,
a supplementary test extracting 96{,}447 new consonantal forms from
Rishonim-era texts (Halakhah, Responsa, Jewish Thought, Kabbalah via the
Sefaria \texttt{hebrew\_library} corpus) yielded only $+58$ additional
decoded matches --- ruling out lexicon period mismatch as the explanation.
A scribal error correction experiment
(\S\ref{sec:scribal-errors}) reinforces this conclusion: single-character
visual corrections recover 10.5\% of unmatched types, but a permutation
control shows this recovery is no better than chance ($z\approx 0$),
confirming that the unmatched gap reflects lexicon inadequacy rather than
copyist noise.

Dependency parsing with DictaBERT further sharpens the diagnosis: the 47
best consecutive phrases (all words DictaLM-validated) score only $2.74/6$
on syntactic quality, barely above random consonant strings ($2.5/6$) and
far below real Hebrew ($6.0/6$). A permutation test confirms that the
actual word order scores no better than random reorderings of the same
vocabulary ($z=-0.51$). The signal is therefore purely lexical: individual
words match Hebrew roots, but their sequential arrangement carries no
syntactic information. This does not resolve the readability gap but
narrows the diagnosis to a precise boundary: real vocabulary, random syntax.
...
\bibitem[Shmidman et~al.(2024a)]{DictaLM2024}
...
\bibitem[Shmidman et~al.(2024b)]{DictaBERTParse2024}
S.~Shmidman, E.~Gueta, et~al.
\newblock {DictaBERT}: A state-of-the-art {H}ebrew {NLP} suite.
\newblock In \emph{Findings of ACL 2024}, pages 4523--4538, 2024.
