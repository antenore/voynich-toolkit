...
across all 227~folios (78.7\% average coverage on 132~herbal pages), while
three independent controls --- random Hebrew words (0/176{,}600), cross-folio
transfer (0/410{,}826), and shuffled positions (0/151{,}000) --- all return
zero matches ($z>100$). Near-match analysis reveals systematic scribal
confusion patterns (vav/resh, tet/tav, ayin/chet) consistent with medieval
Hebrew paleography. The decoded content describes pharmaceutical extraction
recipes with consistent cross-page vocabulary, coherent with the manuscript's
illustrations and with medieval Judeo-Italian medical tradition.
\end{abstract}
...
These are all documented in medieval Hebrew manuscript studies and provide
independent confirmation that the underlying script is handwritten Hebrew.
...
The decoded herbal pages consistently describe pharmaceutical extraction
recipes: filtering through cotton (\heb{mwk}), pouring (\heb{Spk}),
clarifying with chalk (\heb{gyr}), extraction with alcohol (\heb{Skr}),
and a completion test of bitter (\heb{mr}) becoming sweet (\heb{mtq}).
Balneological pages describe therapeutic bathing protocols with immersion
cycles (\heb{SrpEn}) and temperature control. This content is consistent
with medieval Judeo-Italian pharmaceutical manuscripts.
...
A known-plaintext crib attack (\S\ref{sec:crib-attack}) dramatically
strengthens the case: generating plausible Hebrew candidates and encoding
them through the cipher produces 1{,}758~exact matches across 227~folios
(78.7\% average coverage on herbal pages), while three independent controls
(random words, cross-folio, shuffled positions) all return zero matches.
The decoded content --- pharmaceutical recipes with consistent cross-page
vocabulary --- is coherent with the manuscript's illustrations and with
medieval Judeo-Italian medical tradition. A domain-specific lexicon
test (\S\ref{sec:domain-lexicon}) finds one significant result --- balneological
vocabulary concentrates at $2.08\times$ baseline in the bathing section
($z=8.53$) --- but botanical terms show no specialization. The signal is
concentrated in one scribal hand and in continuous text rather than figure
labels.
